...
export function Revisao() {
  ...
  const degraus = ([1, 2, 3, 4] as const).map((caixa) => ({
    caixa,
    dias: INTERVALOS[caixa],
  }));

  return (
    <div className="page">
      <CabecalhoPagina
        rotulo="Memória de longo prazo"
        titulo="Revisão"
        descricao="Revisar no dia certo é o que impede que o que você estudou esta semana desapareça em duas."
        abaixo={
          <>
            <Metricas
              itens={[
                { numero: vencidas.length, rotulo: 'para revisar agora' },
                { numero: agendadas.length, rotulo: 'agendadas nos próximos dias' },
                { numero: Math.max(0, diasRestantes), rotulo: 'dias até o primeiro domingo' },
              ]}
            />
            <div className={s.escada} aria-hidden="true">
              {degraus.map(({ caixa, dias }) => (
                <div key={caixa} className={s.degrau}>
                  <span className={s.degrauNumero}>
                    {dias} {dias === 1 ? 'dia' : 'dias'}
                  </span>
                  <span className={s.degrauBarra} />
                  <span className={s.degrauRotulo}>{caixa}ª revisão</span>
                </div>
              ))}
            </div>
            <p className={s.nota}>
              Os intervalos são de {INTERVALOS[1]}, {INTERVALOS[2]}, {INTERVALOS[3]} e{' '}
      ...
      <div className="container">
        ...
        <section className="secao" aria-labelledby="vencidas">
          ...
          {vencidas.length === 0 ? (
            <Vazio titulo="Nada vencido" acao={<BotaoLink to="/">Ver o plano de hoje</BotaoLink>}>
              <p>
            ...
            <ul className={s.lista}>
              {vencidas.map((item, i) => {
                ...
                return (
                  <li
                    key={item.topicId}
                    className={`${s.item} entrada`}
                    data-area={assunto.areaId}
